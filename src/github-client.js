...
function GitHubClientProvider(props) {
  ...
  //note the initializer callback on useState exposes the client or initializes one
  ...
  /**
   * because hooks allow us to do our setup/teardown in the same callback.
   * see our return cleanup, which returns a function we can call.
   * Also, to make sure it only runs once (mount/unmount), we provide an empty
   * dependency array
   */
  useEffect(() => {
    if (!client) {
